...
from core.pipeline.schemas import DomainDetection, SessionContext
import requests
...
with agent_col_b:
    if detection:
        ...
        st.info(
            f"Agent actif: {active_agent.name} (Auto, confiance {detection.confidence:.2f}) â€” raisons: {reasons_str}"
        )
    else:
...
with col1:
    st.markdown(f"**ðŸ“Š {session.df_name}** â€” {len(df):,} rows Ã— {len(df.columns)} columns")
with col2:
    if session.quality_score:
        if session.quality_score >= 80:
            st.success(f"âœ“ Quality: {session.quality_score:.0f}")
        else:
            st.warning(f"âš ï¸ Quality: {session.quality_score:.0f}")
with col3:
    if st.button("ðŸ“Š Explore", key="agent_explore"):
        st.switch_page("pages/2_ðŸ“Š_Data_Explorer.py")
...
if session.has_data and not session.exchanges and active_agent:
    ...
    if agent_suggestions:
        with st.expander(f"ðŸ’¡ Suggestions de l'agent {active_agent.name}", expanded=True):
...
if 'suggested_question' in st.session_state and st.session_state['suggested_question']:
    ...
    st.info(f"ðŸ’¡ Suggested question: {question}")
...
with st.form(key="agent_question_form", clear_on_submit=False):
    ...
    with col1:
        submit = st.form_submit_button("ðŸš€ Analyze", use_container_width=True, type="primary")
    with col2:
        generate_viz = st.form_submit_button("ðŸ“ˆ + Chart", use_container_width=True)
    with col3:
        export_result = st.form_submit_button("ðŸ“¥ + Export", use_container_width=True)
...
if (submit or generate_viz or export_result) and user_question:
    ...
    st.write("ðŸ” **Debug:** Start processing question")
    ...
    try:
        ...
        st.info(f"ðŸ’¬ Processing question: {question[:50]}...")
        st.write("ðŸ” **Debug:** Entering main try block")
        
        # Detect skills
        detected_skills = detect_skill_from_question(question)
        if detected_skills:
            skills_names = [s['name'] for s in detected_skills]
            st.caption(f"ðŸ› ï¸ Skills detected: {', '.join(skills_names)}")
        ...
        try:
            ...
            if selected_provider in ("codestral", "codestral.ai", "mistral", "mistralai"):
                ...
                if not api_key or api_key == "VOTRE_CLE_CODESRAL_ICI":
                    ...
                    st.info("Set `MISTRAL_API_KEY` in a `.env` file at the project root, or switch provider to LM Studio / Ollama for local usage.")
                    st.code("MISTRAL_API_KEY=your_api_key_here", language="bash")
                    st.stop()

            skills_list = [s['name'] for s in detected_skills] if detected_skills else None
            ...
            if detection:
                pipeline_detection = DomainDetection(
            ...
            session_context = SessionContext(
                ...
                model=llm_model,
                memory_text=context,
                business_context_label=business_context,
            )
            ...
            with st.spinner("Generating code..."):
                pipeline_result = run_pipeline(
                    ...
                    agent_mode=session.selected_agent_mode,
                    selected_agent=active_agent,
                    detection=pipeline_detection,
                    detected_skills=skills_list,
                    data_dictionary=data_dictionary,
                    available_sheets=available_sheets,
                    business_context=business_context,
                    dataset_name=session.df_name,
        ...
        if pipeline_result and pipeline_result.intent_detection and pipeline_result.intent_detection.primary:
            st.caption(f"Intentions detected: {', '.join(pipeline_result.intent_detection.primary[:3])}")

        prompt = pipeline_result.prompt.prompt if pipeline_result and pipeline_result.prompt else ""
        code = pipeline_result.llm_parse.code if pipeline_result and pipeline_result.llm_parse else ""
        exchange["prompt"] = prompt
        exchange["code"] = code

        raw_result = pipeline_result.execution.result if pipeline_result and pipeline_result.execution else None
        validation = pipeline_result.validation.model_dump() if pipeline_result and pipeline_result.validation else None
        formatted = validation['formatted'] if validation else raw_result
        exchange["result"] = formatted
        if validation:
            exchange["validation"] = validation

        fatal_error = pipeline_result and pipeline_result.status in ("blocked", "error") and pipeline_result.error_stage in ("llm_parse", "security")
        if fatal_error:
            message = pipeline_result.error_message or "Unknown error"
            exchange["result"] = f"Error: {message}"
            if pipeline_result.error_stage == "security":
                exchange["auto_comment"] = "Generated code was blocked for security reasons."
            elif pipeline_result.error_stage == "llm_parse":
                exchange["auto_comment"] = "AI did not generate valid code."
            session.add_exchange(exchange)
            st.markdown("---")
            st.markdown("### Response")
            st.error(message)
            if pipeline_result.security and not pipeline_result.security.is_safe and code:
        ...
        if pipeline_result and pipeline_result.error_stage == "execution" and pipeline_result.error_message:
            st.error(f"Execution error: {pipeline_result.error_message}")
            if code:
        ...
        #     try:
        #         with st.spinner("ðŸ§  Professional analysis..."):
        ...
        #     except Exception as e:
        #         st.warning(f"âš ï¸ Error during professional analysis: {str(e)}")
        ...
        try:
            ...
            st.warning(f"âš ï¸ Error saving to database (non-blocking): {str(e)[:100]}")
        
        # Visualization if requested
        if generate_viz and isinstance(formatted, pd.DataFrame) and not formatted.empty:
            with st.spinner("ðŸ“ˆ Generating chart..."):
        ...
        if validation and validation.get('interpretation'):
            st.info(f"ðŸ’¡ {validation['interpretation']}")
        ...
        if isinstance(formatted, pd.DataFrame):
            if not formatted.empty:
                ...
                with col2:
                    if st.button("ðŸ“ˆ Generate Chart", key=f"result_viz_{len(session.exchanges)}"):
                        try:
                ...
                st.warning("âš ï¸ Result is an empty DataFrame.")
        elif formatted is not None:
            st.write(formatted)
        else:
            st.error("âŒ No result to display.")
        
        # Visualization
        if exchange.get("vis_img"):
            st.image(exchange["vis_img"], caption="ðŸ“ˆ Generated visualization")
...
if exchanges:
    ...
    st.info("ðŸ’­ Ask your first question to start the analysis.")
